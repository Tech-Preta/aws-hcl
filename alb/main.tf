# Este arquivo contém a definição de recursos relacionados a um Load Balancer (ALB) na AWS.

# O recurso "aws_lb" representa o Load Balancer em si. Ele é criado apenas se a variável "create" for verdadeira.
# O recurso possui várias configurações, como logs de acesso, logs de conexão, modo de mitigação de desincronização,
# políticas de roteamento de cliente DNS, configurações de segurança, tempo limite, tipo de Load Balancer, entre outros.
# Também é possível definir mapeamentos de sub-redes para o Load Balancer.

# O recurso "aws_lb_listener" representa um listener do Load Balancer. Ele é criado para cada listener definido na variável "listeners".
# Cada listener pode ter várias ações padrão, como autenticação com Cognito, autenticação com OIDC, resposta fixa, encaminhamento,
# encaminhamento ponderado ou redirecionamento. Também é possível configurar autenticação mútua.
# O listener está associado ao Load Balancer através do ARN do Load Balancer.

# Os recursos "aws_lb_listener_rule" representam as regras do listener do Load Balancer. As regras são definidas na variável "listener_rules".
# Cada regra é associada a um listener específico e possui condições e ações. As condições podem ser baseadas em caminhos, cabeçalhos,
# métodos HTTP, entre outros. As ações podem ser redirecionamento, encaminhamento, resposta fixa, autenticação com Cognito ou autenticação com OIDC.

# Além disso, o arquivo contém definições locais e variáveis para facilitar a configuração dos recursos.

# Para mais informações sobre cada recurso e suas configurações, consulte a documentação oficial da AWS.
